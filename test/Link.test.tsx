...
describe('Link', () => {
  ...
  test('dispatches push', () => {
    const dispatch = jest.fn()
    const component = renderComponent(Show({ dispatch }), { to: '/hi' })
    rtl.fireEvent.click(component.getByText('hi'))
    expect(dispatch).toHaveBeenCalledWith(push('/hi'))
  })
  test('calls original onClick', () => {
    ...
    expect(dispatch.mock.calls[0]).toEqual([push('/hi')])
  ...
  describe('errors', () => {
    ...
    test('errors (in dev) on href passed in', () => {
      expect(() =>
        renderComponent(Show({ dispatch: () => {} }), { href: '/hi' }, {}, true)
      ).toThrow(
        'href should not be passed to Link, use "to," "replace" or "route" (passed "/hi")'
      )
    })
  })
  describe('generates the correct path when route option is used', () => {
    ...
    test('replaces route when props change', () => {
      ...
      const component = rtl.render(<S route="hi" there="baby" replace={true} />)
      rtl.fireEvent.click(component.getByText('hi'))
      expect(dispatch).toBeCalledWith(replace('/hi/baby'))
      ...
      component.rerender(<S2 route="there" there="baby" replace={true} />)
      rtl.fireEvent.click(component.getByText('hi'))
