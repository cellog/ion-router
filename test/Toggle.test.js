...
describe('Toggle', () => {
  ...
  describe('initialized', () => {
    ...
    test('does not regenerate the HOC if unnecessary', () => {
      ...
      const First = container.find(R).children().instance()
      expect(container.text()).toBe('')
      container.setProps({ children: 'hi' })
      expect(container.find(R).children().instance()).toBe(First)
      expect(container.text()).toBe('hi')
      container.setProps({ children: 'foo' })
      expect(container.find(R).children().instance()).toBe(First)
    ...
    test('does not re-render if props do not change', () => {
      ...
      expect(container.find(R).children().children().instance().rendered).toBe(1)
      container.setProps({ foo: 'bar' })
      expect(container.find(R).children().children().instance().rendered).toBe(1)
      container.setProps({ foo: 'baz' })
      expect(container.find(R).children().children().instance().rendered).toBe(2)
    })
    test('uses wrapper component', () => {
      ...
      expect(container.find(R).children().children().instance().rendered).toBe(1)
      container.setProps({ wrapper: Wrapper, wrapperProps: { className: 'foo' } })
      expect(container.find(R).children().children().instance().rendered).toBe(1)
      container.setProps({ wrapper: Wrapper, wrapperProps: { className: 'bar' } })
      expect(container.html()).toEqual('<div id="wrapper" class="bar"><div></div></div>')
      expect(container.find(R).children().children().instance().rendered).toBe(1)
    })
  ...
  describe('NullComponent', () => {
    const tests = (type) => {
      ...
      test(`${type} does not re-render if props do not change`, () => {
        ...
        expect(container.children().children().instance().rendered).toBe(1)
        container.setProps({ '@@__loaded': true, '@@__isActive': false })
        expect(container.children().children().instance().rendered).toBe(1)
        container.setProps({ '@@__loaded': true, '@@__isActive': true })
        expect(container.children().children().instance().rendered).toBe(2)
        container.setProps({ '@@__loaded': true, '@@__isActive': true })
        expect(container.children().children().instance().rendered).toBe(2)
      })
    }
