...
describe('Toggle', () => {
  ...
  test("should not freak out if we don't initialize", () => {
    const R = Toggle(() => true)
    expect(() =>
      renderComponent(R, { component: Component, foo: 'bar' }, { week: 1 })
    ).not.toThrow(
      'call connectToggle with the connect function from react-redux to ' +
  ...
  describe('initialized', () => {
    ...
    test('renders the component if the state tester returns true', async () => {
    ...
    test('re-renders if props change', async () => {
      const container = renderComponent(
        Route,
        { component: Component, foo: 'bar' },
        { week: 1 },
      ...
      await rtl.waitForElement(() => container.getByTestId('foo'))
      container.rerender(
        { component: Component, foo: 'baz' },
        { week: 1 },
        false,
        store
      )
      await rtl.waitForElement(() => container.getByText('baz'))
      expect(container.getByTestId('foo')).toHaveTextContent('baz')
      expect(rendered).toBe(2)
    })
    test("does not re-render if props don't change", async () => {
      const container = renderComponent(
        Route,
        { component: Component, foo: 'bar' },
        { week: 1 },
      ...
      await rtl.waitForElement(() => container.getByText('bar'))
      expect(container.getByTestId('foo')).toHaveTextContent('bar')
      expect(rendered).toBe(1)
    })
    test('renders the component if the state tester returns true from props', async () => {
      const container = renderComponent(
        Route,
        { component: Component, foo: 'bar', week: 1 },
        { week: 0 }
      )
      await rtl.waitForElement(() => container.getByTestId('foo'))
      expect(container.getByTestId('foo')).toHaveTextContent('bar')
    ...
    test('componentLoadingMap', () => {
      ...
      expect(container.getByTestId('loadingComponent')).toHaveTextContent(
        '"there"'
      )
